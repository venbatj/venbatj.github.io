...
/**
 * IdentifyLinenRepository
 * 
 * @package Rnt\Masters
 */
class IdentifyLinenRepository implements IRepository
{
    use AERRepository;
    ...
    {
        $Obj = new SqlManager();
        $Obj->addTbls('IdentifyLinen');
        $Obj->addFlds(array('ID', 'IdentifyLinen'));
        $Obj->addOrderFlds('IdentifyLinen', 'ASC');
    ...
    public static function getDataByID($ID)
    {
        $Obj = new SqlManager();
        $Obj->addTbls('IdentifyLinen');
        $Obj->addFlds(array('ID', 'IdentifyLinen', 'Description'));
    ...
    public static function isExists($Row)
    {
        ...
        $Obj->addFldCond('IdentifyLinen', $Row['IdentifyLinen']);    
        $Obj->addFldCond('Flag', 'R', '!=');
	...
	public static function getDataTblListCount($SearchTxt) 
	{
		...
		$Obj->addFldCond('Flag', 'R', '!=');
		if ($SearchTxt != '') {
			$Obj->addFldCond('IdentifyLinen', "%{$SearchTxt}%", 'LIKE', 'AND', '(');
