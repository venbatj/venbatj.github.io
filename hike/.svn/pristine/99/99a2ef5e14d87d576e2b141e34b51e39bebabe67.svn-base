 ...
 * @package Rnt\Masters
 */
class InventoryMoveRepository implements IRepository
{
    use AERRepository;
    ...
    {
        $Obj = new SqlManager();
        $Obj->addTbls('InventoryMove');
        $Obj->addFlds(array('ID', 'InventoryMove'));
        $Obj->addOrderFlds('InventoryMove', 'ASC');
    ...
    public static function getDataByID($ID)
    {
        $Obj = new SqlManager();
        $Obj->addTbls('InventoryMove');
        $Obj->addFlds(array('ID', 'InventoryMove', 'Description'));
    ...
    public static function isExists($Row)
    {
        ...
        $Obj->addFldCond('InventoryMove', $Row['InventoryMove']);    
        $Obj->addFldCond('Flag', 'R', '!=');
	...
	public static function getDataTblListCount($SearchTxt) 
	{
		...
		$Obj->addFldCond('Flag', 'R', '!=');
		if ($SearchTxt != '') {
			$Obj->addFldCond('InventoryMove', "%{$SearchTxt}%", 'LIKE', 'AND', '(');
