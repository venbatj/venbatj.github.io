 ...
 * @package Rnt\Masters
 */
class CustomerServiceRepository implements IRepository
{
    use AERRepository;
    ...
    {
        $Obj = new SqlManager();
        $Obj->addTbls('CustomerService');
        $Obj->addFlds(array('ID', 'CustomerService'));
        $Obj->addOrderFlds('CustomerService', 'ASC');
    ...
    public static function getDataByID($ID)
    {
        $Obj = new SqlManager();
        $Obj->addTbls('CustomerService');
        $Obj->addFlds(array('ID', 'CustomerService', 'Description'));
    ...
    public static function getDataByName($Name)
    {
        ...
        $Obj->addTbls('CustomerService');
        $Obj->addFlds(array('ID', 'CustomerService', 'Description'));
        $Obj->addFldCond('LOWER(TRIM(CustomerService))', $Name);
        $Obj->addFldCond('Flag', 'R', '!=');
        return $Obj->getSingle();
    }
    ...
    public static function isExists($Row)
    {
        ...
        $Obj->addFldCond('CustomerService', $Row['CustomerService']);    
        $Obj->addFldCond('Flag', 'R', '!=');
	...
	public static function getDataTblListCount($SearchTxt) 
	{
		...
		$Obj->addFldCond('Flag', 'R', '!=');
		if ($SearchTxt != '') {
			$Obj->addFldCond('CustomerService', "%{$SearchTxt}%", 'LIKE', 'AND', '(');
	...
	/**
     * Get data table liCustomerService
	 * @param int $IndexCustomerService
	 * @param int $LimitCustomerService
	 * @param string $SearchTxt
