...
use Rnt\Masters\CountryRepository as CR;
...
class Country
{
     ...
     * @return array
     */
    public static function submitFeedback($Req)
    {
        if (isset($Req['ID']) && $Req['ID'] > 0)
            return Utils::response(CR::updateFeedback($Req['data']), true);
        else
            return Utils::response(CR::insertFeedback($Req['data']), true);
    }
    /**
    ...
    {
        if (CR::isExists($Req))
            return Utils::errorResponse('Country "'.$Req['CountryName'].'" already exists!');
