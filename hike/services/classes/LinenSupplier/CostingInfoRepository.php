...
class CostingInfoRepository implements IRepository
{
    ...
    public static function isExistLinen($Row)
    {
        ...
        $Obj->addFldCond('LinenSupplierID', $Row['LinenSupplierID']);    
     ...
     * @return int
     */
	public static function getDataTblListCount($SearchTxt, $LinenSupplierID) 
	{
		$Obj = new SqlManager();
		$Obj->addTbls('LinenSupplierCosting LSC');
		$Obj->addFlds(array('COUNT(#LSC.ID#) RowCount'));
        $Obj->addFldCond('LSC.Flag', 'R', '!=');
        $Obj->addFldCond('LSC.LinenSupplierID', $LinenSupplierID);
	...
	public static function getDataTblList($LinenSupplierID, $Index, $Limit, $SearchTxt = '', $OrderFld = '', $OrderType = '')
	{
        ...
        $Obj->addFldCond('LSC.Flag', 'R', '!=');
        $Obj->addFldCond('LinenSupplierID', $LinenSupplierID);
        if ($SearchTxt != '') {
            $Obj->addFldCond('CategoryName', "%{$SearchTxt}%", 'LIKE', 'AND', '(');
            $Obj->addFldCond('ProductName', "%{$SearchTxt}%", 'LIKE', 'OR');
            $Obj->addFldCond('VariantName', "%{$SearchTxt}%", 'LIKE', 'OR');
