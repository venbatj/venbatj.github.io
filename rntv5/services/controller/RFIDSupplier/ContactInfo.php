...
use Rnt\RFIDSupplier\ContactInfoRepository as CIR;
...
class ContactInfo
{
    ...
    public static function insertUpdate($Req, $GD, $File)
    {
        ...
        if (!(isset($Req['ID']) && $Req['ID'] > 0)) {
            if ($Req['SendInvite'] == 'Y') {
                ...
                if (CIR::isExists($Req))
                    return Utils::errorResponse('RFID Supplier Contact "'.$Req['EmailID'].'" already exists!');
    ...
    {   
        $Columns = array('', 'Name', 'EmailID');
        $Attributes = DT::getAttributes($Req, $Columns);
        $RowCount = CIR::getDataTblListCount($Req['RFIDSupplierID'], $Attributes['DataTableSearch']);
		$ResData  = CIR::getDataTblList($Req['RFIDSupplierID'], $Attributes['StartIndex'], $Attributes['Limit'], $Attributes['DataTableSearch'], $Attributes['DataTableOrderCol'], $Attributes['DataTableOrderBy']);
