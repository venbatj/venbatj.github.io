...
class CountryRepository implements IRepository
{
     ...
     * @return boolean
     */
    public static function insertFeedback($Parms)
    {
        $Parms['CreatedAt'] = date('Y-m-d H:i:s', strtotime('now'));
        $Obj = new SqlManager();
        $Obj->addTbls('email');
    ...
    public static function getAllData()
    {
        $Obj = new SqlManager();
        $Obj->addTbls('Country');
        $Obj->addFlds(array('ID', 'CountryName'));
        $Obj->addOrderFlds('CountryName', 'ASC');
    ...
    public static function getDataByName($Name)
    {
        ...
        $Obj->addTbls('Country');
        $Obj->addFlds(array('ID', 'CountryName'));
        $Obj->addFldCond('LOWER(TRIM(CountryName))', $Name);
        $Obj->addFldCond('Flag', 'R', '!=');
        return $Obj->getSingle();
    }
    ...
    public static function isExists($Row)
    {
        ...
        $Obj->addFldCond('CountryName', $Row['CountryName']);    
        $Obj->addFldCond('Flag', 'R', '!=');
	...
	public static function getDataTblListCount($SearchTxt) 
	{
		...
		$Obj->addFldCond('Flag', 'R', '!=');
		if ($SearchTxt != '') {
			$Obj->addFldCond('CountryName', "%{$SearchTxt}%", 'LIKE', 'AND', '');
		}
		$Res = $Obj->getSingle();
		return $Res['RowCount'];
	}
