...
use Rnt\RFIDSupplier\BillingInfoRepository as BIR;
...
class BillingInfo
{
    ...
    public static function insertUpdate($Req, $GD, $File)
    {
        ...
        if (!(isset($Req['ID']) && $Req['ID'] > 0)) {
            if ($Req['SendInvite'] == 'Y') {
                ...
                if (BIR::isExists($Req))
                    return Utils::errorResponse('RFID Supplier Billing "'.$Req['EmailID'].'" already exists!');
    ...
    {   
        ...
        $RowCount = BIR::getDataTblListCount($Req['RFIDSupplierID'], $Attributes['DataTableSearch']);
		$ResData  = BIR::getDataTblList($Req['RFIDSupplierID'], $Attributes['StartIndex'], $Attributes['Limit'], $Attributes['DataTableSearch'], $Attributes['DataTableOrderCol'], $Attributes['DataTableOrderBy']);
