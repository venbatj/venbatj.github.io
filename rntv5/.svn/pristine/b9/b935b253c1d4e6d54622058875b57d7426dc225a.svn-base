...
class ContractInfoRepository implements IRepository
{
    ...
    {
        $Obj = new SqlManager();
        $Obj->addTbls('LinenSupplierContract');
    ...
    public static function getDataByID($ID)
    {
        ...
        $Obj->addTbls('LinenSupplierContract');
        $Obj->addFlds('*');
        $Obj->addFldCond('LinenSupplierID', $ID);
        $Obj->addFldCond('Flag', 'R', '!=');
    ...
    public static function isExists($Row)
    {
        $Obj = new SqlManager();
        $Obj->addTbls('LinenSupplierContract');
        $Obj->addFlds(array('LinenSupplierID'));        

        if (isset($Row['ID']) && $Row['ID'] > 0)
            $Obj->addFldCond('LinenSupplierID', $Row['ID'], '!=');
	...
	{
		$Obj = new SqlManager();
		$Obj->addTbls('LinenSupplierContract LS');
