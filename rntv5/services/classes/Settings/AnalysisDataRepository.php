 ...
 */
class AnalysisDataRepository implements IRepository
{
    use AERRepository;
    ...
    public static function getCostParameters()
    {
        ...
        $Obj->addFlds(array('ID', 'DollarConversionRate', 'CostRFIDTag', 'DutiesLocalTransportation', 'BenefitsOverheads',
    ...
    //     // $Obj->addFldCond('EffectiveFrom', date("Y-m-d", strtotime($Row['EffectiveFrom'])));
    //     $Obj->addFldCond('Flag', 'R', '!=');
	...
	public static function getDataTblListCount($SearchTxt) 
	{
        ...
        if ($SearchTxt != '') {
            $Obj->addFldCond('TotalDeliveryCost', "%{$SearchTxt}%", 'LIKE', 'AND', '(');
	...
	public static function getDataTblList($Index, $Limit, $SearchTxt = '', $OrderFld = '', $OrderType = '')
    {
		...
		$Obj = new SqlManager();
		$Obj->addTbls('DeliveryTechCostAnalysis');
		$Obj->addFlds(array('ID','TotalDeliveryCost',' TechCost', $Status.' Status'));
        $Obj->addFldCond('Flag', 'R', '!=');
