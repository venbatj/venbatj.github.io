...
use Rnt\LinenSupplier\ContactInfoRepository as CIR;
...
class ContactInfo
{
    ...
    public static function insertUpdate($Req, $GD, $File)
    {
        ...
        if (!(isset($Req['ID']) && $Req['ID'] > 0)) {
            if ($Req['SendInvite'] == 'Y') {
                ...
                if (CIR::isExists($Req))
                    return Utils::errorResponse('Linen Contact "'.$Req['EmailID'].'" already exists!');
    ...
    {   
        $Columns = array('', 'Name', 'EmailID','Phone');
        $Attributes = DT::getAttributes($Req, $Columns);
        $RowCount = CIR::getDataTblListCount($Req['LinenSupplierID'], $Attributes['DataTableSearch']);
		$ResData  = CIR::getDataTblList($Req['LinenSupplierID'], $Attributes['StartIndex'], $Attributes['Limit'], $Attributes['DataTableSearch'], $Attributes['DataTableOrderCol'], $Attributes['DataTableOrderBy']);
